...
-(BOOL)filterType:(NSString*)type inputValue:(NSString*)value length:(int)length maxLength:(int)maxLength {
    ...
    NSCharacterSet *cs = [[NSCharacterSet characterSetWithCharactersInString:type] invertedSet];
    NSString *filtered = [[value componentsSeparatedByCharactersInSet:cs] componentsJoinedByString:@""];
    return [self isEqualToString:filtered] && (length < maxLength) ;
}
